...
files = {
    'zip':    os.path.join('build-cp', f'{repo_name}-{commit}.zip'),
    'readme': os.path.join('build-cp', repo_name, 'README.txt'),
}
dirs = {
    'cache':  CACHE_DIR,
    'root':   os.path.join('build-cp', repo_name),
    '9.x':    os.path.join('build-cp', repo_name, 'CircuitPython 9.x'),
    '9_lib':  os.path.join('build-cp', repo_name, 'CircuitPython 9.x', 'lib'),
}
...
# Make the zip file
run(f"cd build-cp; zip -r {basename(files['zip'])} {basename(dirs['root'])}")
